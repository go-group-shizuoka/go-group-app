...
/**
 * File/Blob → 生base64（プレフィックスなし）。
 * FileReader ではなく arrayBuffer を使うのでブラウザ・Node の両方で動く（＝テストできる）。
 */
export async function ocrFileToBase64(file) {
  const buf = new Uint8Array(await file.arrayBuffer());
  if (typeof btoa === "function") {
    let s = "";
    const CHUNK = 0x8000;                       // 引数上限を超えないよう分割して連結
    for (let i = 0; i < buf.length; i += CHUNK) {
      s += String.fromCharCode.apply(null, buf.subarray(i, i + CHUNK));
    }
    return btoa(s);
  }
  return Buffer.from(buf).toString("base64");   // Node（テスト実行時）
}

/**
 * アップロード前処理の共通入口。
 *  - 画像: ocrCompressImage で JPEG 化＋上限内に縮小（HEIC/PNG/巨大写真もここで吸収）
 *  - PDF : 圧縮できないのでそのまま base64 化し、上限超過だけを送信前に弾く
 * どちらも「関数に到達しないまま Vercel が 413 を返す」状態を未然に防ぐのが目的。
 * @returns {Promise<{base64:string, dataUrl:string, mediaType:string, isPdf:boolean}>}
 */
export async function ocrPrepareUpload(file, { maxPx = 1600, quality = 0.82 } = {}) {
  if (!file) throw ocrError("画像の準備", "ファイルが選択されていません。");
  const isPdf = (file.type || "") === "application/pdf";
  if (!isPdf) {
    const dataUrl = await ocrCompressImage(file, { maxPx, quality });
    const base64  = dataUrl.slice(dataUrl.indexOf(",") + 1);
    if (!base64) throw ocrError("画像の準備", "画像を読み取れませんでした。もう一度撮影してください。");
    return { base64, dataUrl, mediaType: "image/jpeg", isPdf: false };
  }
  const base64 = await ocrFileToBase64(file);
  if (base64.length > OCR_MAX_B64_BYTES) {
    throw ocrError("画像の準備",
      "PDFのサイズが大きすぎます。ページを分けるか、書類を写真で撮影してください。",
      `pdfB64=${base64.length}`);
  }
  return { base64, dataUrl: `data:application/pdf;base64,${base64}`, mediaType: "application/pdf", isPdf: true };
}
